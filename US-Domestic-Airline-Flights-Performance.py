...
# Load data from airline reporting dataset into pandas dataframe
...
# Convert Year, Quarter and Month to string
years = [i for i in range(2005, 2021, 1)]


# Compute components
# Yearly airline performance report and Annual average flight delays


def compute_flight_performance(df):
    # Compute yearly airline performance
    # Total cancelled flights per cancellation category
    bar_data = df.groupby(['Month', 'CancellationCode'])[
        'Flights'].sum().reset_index()

    # Average flight time per reporting airline
    line_data = df.groupby(['Month', 'Reporting_Airline'])[
        'AirTime'].mean().reset_index()

    # Diverted landings per airport
    pie_data = df[df['DivAirportLandings'] != 0.0]

    # Sum of flight from states
    map_data = df.groupby(['OriginState'])[
        'Flights'].sum().reset_index()

    # Sum of flight to destination states
    tree_data = df.groupby(['DestState', 'Reporting_Airline'])[
        'Flights'].sum().reset_index()

    return bar_data, line_data, pie_data, map_data, tree_data


def compute_flight_delay(df):
    # Compute average delay times
...
app.layout = html.Div(children=[
    ...
    html.Div([
        # Add an division
        html.Div([
            # Create an division for adding dropdown helper text for report
            # type
            html.Div(
                [
        ...
        # Add next division
        html.Div([
            # Create an division for adding dropdown helper text for choosing
            # year
            html.Div(
                [
    ...
    # Add Computed graphs
    # REVIEW3: Observe how we add an empty division and providing an id that
    # will be updated during callback
    html.Div([], id='plot1'),
    ...
    # TASK3: Add a division with two empty divisions inside. See above division
    # for example.
    # Enter your code below. Make sure you have correct formatting.
    html.Div([
        html.Div([], id='plot4'),
...
@app.callback([Output(component_id='plot1', component_property='children'),
              ...
              # REVIEW4: Holding output state till user enters all the form
              # information. In this case, it will be chart type and year
...
def get_graph(chart, year, children1, children2, c3, c4, c5):
    ...
    if chart == 'OPT1':
        ...
        bar_data, line_data, pie_data, map_data, tree_data = \
            compute_flight_performance(df)
        ...
        line_fig = px.line(line_data, x='Month', y='AirTime',
                           ...
                           title='Average Flight Time By Reporting Airline')

        # Percentage of diverted airport landings per reporting airline
        pie_fig = px.pie(pie_data, values='Flights',
        ...
        map_fig = px.choropleth(map_data,  # Input data
                                ...
                                range_color=[0, map_data[
                                    'Flights'].max()])
        map_fig.update_layout(
            title_text='Number of flights from origin state',
            geo_scope='usa')  # Plot only the USA instead of globe

        # TASK6: Number of flights flying to each state from each reporting
        # airline
        # Enter your code below. Make sure you have correct formatting.
        ...
        # REVIEW7: This covers chart type 2 and we have completed this exercise
        # under Flight Delay Time Statistics Dashboard section
        # Compute required information for creating graph from the data
        avg_car, avg_weather, avg_nas, avg_sec, avg_late = compute_flight_delay(
            df)

        # Create graph
        carrier_fig = px.line(avg_car, x='Month', y='CarrierDelay',
                              color='Reporting_Airline',
                              title='Average carrier delay time (minutes) by'
                                    '/airline')
        weather_fig = px.line(avg_weather, x='Month', y='WeatherDelay',
                              color='Reporting_Airline',
                              title='Average weather delay time (minutes) by'
                                    '/airline')
        ...
        sec_fig = px.line(avg_sec, x='Month', y='SecurityDelay',
                          color='Reporting_Airline',
                          title='Average security delay time (minutes) by'
                                '/airline')
        late_fig = px.line(avg_late, x='Month', y='LateAircraftDelay',
                           color='Reporting_Airline',
                           title='Average late aircraft delay time (minutes)'
                                 '/by airline')
